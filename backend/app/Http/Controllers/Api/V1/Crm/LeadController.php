...
final class LeadController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($search=trim((string)$request->query('search',''))){$query->where(fn($q)=>$q->where('name','ilike',"%{$search}%")->orWhere('company_name','ilike',"%{$search}%")->orWhere('email','ilike',"%{$search}%"));}
    ...
    public function update(LeadRequest $request,string $lead,AuditLogger $audit): LeadResource
    {
        ...
        $model=$this->findLead($lead);$model->update($request->validated());
    ...
    private function findLead(string $publicId): CrmLead{return CrmLead::query()->where('public_id',$publicId)->firstOrFail();}
}
