...
function crmUser(Tenant $tenant): User
{
    ...
    foreach($codes as $code){ Permission::query()->firstOrCreate(['code'=>$code],['name'=>$code,'module_code'=>'crm']); }
    ...
    $user=User::factory()->create(['tenant_id'=>$tenant->id,'status'=>UserStatus::Active]);$user->roles()->attach($role->id);
...
it('creates and lists a tenant customer', function () {
    $tenant=Tenant::factory()->create();$user=crmUser($tenant);$token=$user->createToken('test')->plainTextToken;
...
it('does not expose another tenant crm customer', function () {
    $a=Tenant::factory()->create();$b=Tenant::factory()->create();$user=crmUser($a);$token=$user->createToken('test')->plainTextToken;
